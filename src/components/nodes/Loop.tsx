...
import { Repeat, CheckCircle2, CircleDashed, Loader2, AlertCircle } from 'lucide-react';
...
const getStatusStyles = (status: NodeStatus) => {
  switch (status) {
    ...
    case 'running':
      return 'border-orange-500 bg-zinc-800 text-orange-100 shadow-[0_0_10px_rgba(249,115,22,0.3)]';
    case 'completed':
...
export function Loop({ data, selected }: NodeProps<Node<BaseNodeData>>) {
  return (
    <div
      className={cn(
        ...
        selected ? 'ring-2 ring-orange-500 ring-offset-2 ring-offset-zinc-900' : 'hover:shadow-lg hover:shadow-orange-500/20'
      )}
    >
      {/* Standard Input */}
      <Handle type="target" position={Position.Left} id="input" className="w-4 h-4 bg-zinc-400 border-2 border-zinc-800 hover:scale-125 hover:bg-indigo-400 transition-transform" />
      
      {/* Loop Return Output (Top) - Source handle to send retry signal */}
      <Handle 
        type="source" 
      ...
      >
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 pointer-events-none">
          <span className="text-[10px] font-bold text-orange-500 opacity-0 group-hover:opacity-100 transition-opacity uppercase whitespace-nowrap">Retry Out</span>
        </div>
      </Handle>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs opacity-70">Loop Controller</div>
        </div>
      ...
      <Handle type="source" position={Position.Right} id="output" className="w-4 h-4 bg-zinc-400 border-2 border-zinc-800 hover:scale-125 hover:bg-indigo-400 transition-transform" />
    </div>
  );
}
