...
import { Zap, CheckCircle2, CircleDashed, Loader2, AlertCircle } from 'lucide-react';
...
export function Skill({ data, selected }: NodeProps<Node<BaseNodeData>>) {
  return (
    <div
      className={cn(
        ...
        selected ? 'ring-2 ring-amber-500 ring-offset-2 ring-offset-zinc-900' : 'hover:shadow-lg hover:shadow-amber-500/20'
      )}
    >
      <Handle type="target" position={Position.Left} className="w-5 h-5 bg-zinc-400 border-2 border-zinc-800 hover:scale-125 hover:bg-amber-400 transition-transform" />
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-zinc-700/50 text-amber-400">
          <Zap className="w-5 h-5" />
        </div>
        
        <div className="flex-1">
          <div className="text-sm font-medium">{data.label}</div>
          <div className="text-xs opacity-70">Skill</div>
        </div>
      ...
      <Handle type="source" position={Position.Right} className="w-5 h-5 bg-zinc-400 border-2 border-zinc-800 hover:scale-125 hover:bg-amber-400 transition-transform" />
    </div>
  );
}
