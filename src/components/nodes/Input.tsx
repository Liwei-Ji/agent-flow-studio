...
import { Play, CheckCircle2, CircleDashed, Loader2, AlertCircle } from 'lucide-react';
...
export function Input({ data, selected }: NodeProps<Node<BaseNodeData>>) {
  return (
    <div
      className={cn(
    ...
    >
      <div className="flex items-center gap-3">
        <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-zinc-500/10 text-zinc-400">
          <Play className="w-4 h-4 fill-zinc-400/20" />
        </div>

        <div className="flex-1">
          <div className="text-sm font-semibold tracking-tight">{data.label}</div>
          <div className="text-[10px] uppercase font-bold opacity-40 tracking-wider">Input Source</div>
        </div>
      ...
      <Handle type="source" position={Position.Right} className="w-3 h-3 bg-zinc-600 border-2 border-zinc-800 hover:scale-125 hover:bg-indigo-400 transition-all" />
    </div>
  );
}
